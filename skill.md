...
4. **核实描述一致性** → 将用户描述的牌名、关键字异能、游戏动作与实际牌面数据对比；若发现出入，**停止推进**，先向用户确认（详见「牌张描述核实与用户确认」章节）
...
通过 mtgch + Scryfall 查询后，若发现以下任一情况，必须在回答前向用户确认：

| 出入类型 | 说明 | 示例 |
|----------|------|------|
| **牌名查无结果** | 用户输入的牌名在 mtgch 中无匹配或返回空结果 | 输入「雷霆巨龙」但数据库中无此牌 |
| **异能名称错误** | 用户提到的关键字与牌面实际关键字不符 | 称 Hell's Thunder（狱雷妖）有「逸脱」，实际为「破坟」|
| **动作描述错误** | 用户描述的游戏动作不是该牌的合法操作 | 称某牌"可以从牌库直接逸脱" |
| **牌名混淆** | 用户提到的牌名与所描述效果不匹配，疑似误认同名或相似牌 | 用「血月」描述乌尔博格的效果 |
| **机制适用场景有误** | 描述的施放/使用方式不适用于该牌，但可能经其他牌/效应间接实现 | "从放逐区延缓一张不具备 Suspend 的牌" |

### 可选流程：牌名/异能名无法匹配时的近似推荐

当用户输入的牌名或异能名在 mtgch 中**找不到精确匹配**，或查询结果与用户描述明显不符时，执行以下流程：

#### 步骤1：尝试模糊搜索

```bash
# 使用用户输入的关键词进行模糊搜索，获取多个候选结果
curl -s "https://mtgch.com/api/v1/autocomplete/?q={用户输入的牌名或关键词}&is_for_deck=false&size=5&page=1" \
  -H "accept: application/json" | head -c 5000
```

#### 步骤2：筛选近似候选

从搜索结果中，根据以下维度筛选 **2-3 个最可能的候选牌**：
- **名称相似度**：中文名或英文名与用户输入接近
- **效果匹配度**：牌面异能与用户描述的效果/场景吻合
- **使用频率**：优先推荐常见赛制中出现率较高的牌
...
**API 查询结果**：mtgch 中无「雷霆践踏」精确匹配。

**Agent 回复**：
> 我在数据库中未找到名为「雷霆践踏」的牌。根据你的描述，你可能是指以下其中一张：
>
> 1. **雷蹄巴洛西 (Thunderfoot Baloth)** — 6费绿色生物，具有践踏；你操控指挥官时所有生物+2/+2并获得践踏
> 2. **雷霆挥击 (Thunder Strike)** — 2费红色瞬间，目标生物+2/+0并获得先攻直到回合结束
> 3. **雷霆之怒 (Thunderous Wrath)** — 6费红色瞬间，造成5点伤害，具有神迹费用{R}
>
> 请问你指的是哪一张？或者你能补充更多该牌的特征（颜色、费用、类别等）帮助我定位。

#### 注意事项

- **不要猜测作答**：在未确认具体牌张前，不要基于猜测提供规则裁定
- **优先贴合用户描述**：推荐候选时，应优先选择与用户描述的效果/场景最相关的牌
- **允许用户补充信息**：如果候选都不对，引导用户提供更多线索（如颜色、费用、系列、效果关键词等）
- **确认后立即继续**：用户选定后，回到正常回答流程继续分析

### 处理方式

1. **明确指出**数据库中该牌的实际异能/规则文本
2. **提出两种或以上可能的情况**，让用户自行确认其真实意图
3. **不要直接否定用户**，应考虑其描述可能来自其他牌的赋能（如冥界裂隙、逃脱地等）
4. 用户确认意图后，再继续正常回答流程

### 对话示例

**用户问题**：
> 我从坟场逸脱狱雷妖（Hell's Thunder），对手可以施放反击咒语反击吗？

**API 查询结果**：Hell's Thunder（狱雷妖）的 oracle_text 含「Unearth {R}」，无「Escape」关键字。

**Agent 回复**：
> 根据牌面资料，**狱雷妖（Hell's Thunder）** 具备的坟场异能是 **破坟（Unearth）** 而非逸脱（Escape）。请问你是：
> 1. 将「破坟」与「逸脱」混淆，实际想问的是破坟触发后对手能否响应？
> 2. 还是场上有**冥界裂隙（Underworld Breach）** 或其他能给予逸脱的效应，使狱雷妖可以以逸脱模式施放？
>
> 请确认情境后我再为你解答。

### 注意事项

- **不要因此跳过回答**：确认后应立即继续完整的分析流程
- **可能是真实的组合技**：用户描述"不符常规"不代表一定错误，需给出组合路径的可能性
- **同名陷阱**：查到多张同名牌时，应列出所有版本，请用户指定
